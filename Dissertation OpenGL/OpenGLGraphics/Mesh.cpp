...
Mesh* Mesh::GenerateSquare()
{
	...
	m->numIndices = 6;
	uint ind[] = { 0, 1, 2, 2, 1, 3 };
	m->indices = ind;

	m->colours = new Vector4[m->numVertices];
	m->colours[0] = Vector4(0.5f, 0.5f, 0.5f, 1.0f);
	m->colours[1] = Vector4(0.5f, 0.5f, 0.5f, 1.0f);
	m->colours[2] = Vector4(0.5f, 0.5f, 0.5f, 1.0f);
	m->colours[3] = Vector4(0.5f, 0.5f, 0.5f, 1.0f);
...
Mesh* Mesh::GenerateLineLoop(std::vector<Vector3> &points) {
	Mesh *m = new Mesh();

	int pointCount = points.size();
	m->type = GL_LINE_LOOP;
	m->numVertices = pointCount;
	m->vertices = new Vector3[pointCount];
	m->colours = new Vector4[pointCount];

	for (int i = 0; i < pointCount; i++) {
		m->vertices[i] = points[i];
		m->colours[i] = Vector4(0, 0, 0, 1);
		if (i % 2 == 1)
		{
...
Mesh* Mesh::GeneratePoints(std::vector<Vector3> &points) {
	Mesh *m = new Mesh();
